        ...
        <div className="space-y-1">
          {bars.map((b, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="w-28 shrink-0 truncate text-right font-mono text-[9px] text-slate-500">{b.label}</span>
              ...
              <span className="w-8 shrink-0 font-mono text-[9px]" style={{ color: barColor(b.value) }}>{b.value.toFixed(1)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {Array.isArray(out?.numbers_used) && out!.numbers_used!.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {out!.numbers_used!.slice(0, 6).map((num, i) => {
            ...
            return (
              <span key={i} className={`rounded border px-1.5 py-0.5 font-mono text-[9px] ${
      ...
      {dim ? (
        <div className="rounded-md border border-line bg-panel-2 p-2">
          ...
          <div className="mt-1 flex items-center justify-between font-mono text-[10px]">
            <span className="text-slate-500">→ verdict {projected.toFixed(1)}/10</span>
            <span className={delta > 0.02 ? "text-ok" : delta < -0.02 ? "text-err" : "text-slate-500"}>
              {delta >= 0 ? "▲ +" : "▼ "}{delta.toFixed(2)} via {dim}
            </span>
            {override !== null && (
              <button onClick={() => setOverride(null)} className="text-slate-500 hover:text-cyan">↺ reset</button>
            )}
          </div>
        </div>
      ) : (
